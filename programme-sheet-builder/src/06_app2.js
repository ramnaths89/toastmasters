...
function buildSheetHTML(interactive){
  ...
  /* The Contest Committee block (V43). Both of Rama's reference contest sheets
     put the committee in the left column, not in an agenda row, and that is the
     right place for it: it is who is running the night, referred to throughout,
     not something that happens at 7:20. It replaces the Executive Committee for
     the same reason - during a contest the Exco hold no contest appointment, and
     printing both invites the reader to look for the wrong name.
     An unfilled appointment prints TBD in the pane exactly as it does in the
     Appointment Holder column, so the gap is visible on the paper the committee
     is reading from.
     Custom roles are appended after the built-in appointments, exactly as they
     are appended to the chapter roster. Without that a club role added in
     contest mode was collected, chased as "still open" and saved to the file
  ...
  const committeeRows = Object.entries(CONTEST_ROLE_LABELS)
    ...
    (name
      ? `<div class="exco-name">${esc(name)}</div>`
      : `<div class="exco-name"><span class="tbd-inline">TBD</span></div>`) + `</div>`).join('');

  /* Emitted twice — once per printed page (see the .ref-pane note in sheet.css).
     The Pathways legend is chapter-only: a contest schedules no Pathways project,
     so the legend would be eleven lines explaining abbreviations that appear
     nowhere on the sheet. Dropping it is also what makes room for the committee. */
  const paneInner = isContest() ? `
      <div class="exco-block contest-committee"><h3>Contest Committee</h3>${committeeRows}</div>
  ...
  const tableRows = rows.map(({seg,clock}, idx)=>{
    ...
    /* A contest block lists its contestants in draw order under the title (V43).
       Numbered, because the draw order is the running order and the number is
       what the Chief Judge and the tally counters call out. An empty list prints
       the "no contestants yet" line rather than a bare heading, so a half-built
       sheet cannot be mistaken for a contest nobody entered.
       Blank entries are kept and numbered: a confirmed slot with an unconfirmed
...
/* V43: the CONTEST pane is a different object - the Contest Committee (built
   from the contest appointments and their ticks) replaces the Exco, and the
   Pathways legend is dropped. Leaving those out meant the "already measured this
   signature" shortcut fired against a number taken from the chapter pane: the
   warning stayed hidden while a committee of thirteen long names overflowed the
   printed page by about 6 mm. If a block can change the pane's HEIGHT it belongs
   here, and the cost of listing something that cannot is one extra measurement. */
function paneFitSignature(){
  return [state.execText, state.districtText, state.linksText,
          state.announcementsText, state.theme,
          state.meeting.mode,
          /* Only the contest appointments and their ticks move the contest pane,
             but serialising the whole roles object is cheap and cannot go stale
             the way a hand-picked subset of keys would. */
          JSON.stringify(state.roles), JSON.stringify(state.roleActive),
          JSON.stringify(state.customRoles)].join('\u0000');
}
...
async function renderSheetParts(opts){
  ...
  try{
    ...
    if(paginate){
      ...
      /* THE JPG CUT (V44). The footer is out of flow - the fix stylesheet above
         pins it position:absolute; bottom:0 - so it contributes nothing to
         page.scrollHeight, and natural() is therefore the height of the content
         ALONE. Painting it at the bottom of a page that tall lands the opaque
         footer band ON TOP of the last row rather than under it, and the image
         ends mid-row.
         The PDF never showed this because pagination rounds the page up to whole
         sheets, which leaves slack below the last row by construction. The JPG
         does not paginate on purpose (a single image would just gain a band of
         white), so it has to make the room explicitly.
         Measured on Rama's 27 Aug sheet: content ended at 1690, the canvas was
         cut at 1674 - the missing 16 px is the overlap, and the row it ate was
         "Club President Adjourns Meeting". */
      const footH = foot ? foot.offsetHeight : 0;
      if(footH){
        h = natural() + footH;
        page.style.minHeight = h + 'px';
        await new Promise(r=>setTimeout(r, 40));
        /* Growing the page grows the pane's percentage min-height with it, so
           re-read rather than trusting the number we just set. */
        h = Math.max(h, natural());
      }
    }
...
function toggleRoleActive(key, on){
  ...
  if(!on && dropped){
    /* roleLabelMap(), not ROLE_LABELS: contest appointments and custom roles are
       both absent from the built-in nine and the banner read "undefined switched
       off" for either of them. */
    showBanner(`${roleLabelMap()[key] || 'Role'} switched off — ${dropped} agenda item${dropped>1?'s':''} removed from the programme. Tick it again to bring them back.`, false);
  }
}

/* ================= Meeting mode (V43) =================
   Switching mode REPLACES the running order, because the two running orders have
   nothing in common - there is no useful merge between "TME Welcome Remarks" and
   "Briefing for Judges". Everything else survives: club setup, the officers, the
   links, the announcements, and BOTH role sets, so switching to a contest and
   back does not cost the chapter roster.
   It asks first, and only when there is something to lose. A running order still
   identical to its template is replaced silently - refusing to touch it would
   mean a confirm dialog on the first click of a fresh sheet, which trains people
   to dismiss the one that matters. */
function setMeetingMode(mode){
  const next = (mode === 'contest') ? 'contest' : 'chapter';
  const cur = state.meeting.mode === 'contest' ? 'contest' : 'chapter';
  if(next === cur) return;
  if(!runningOrderIsPristine(cur)){
    const what = next === 'contest' ? 'contest running order' : 'chapter-meeting running order';
    if(!confirm('Switch to the ' + what + '?\n\n'
      + 'The programme segments on screen will be replaced. Your club setup, '
      + 'officers, announcements and both sets of role names are kept.')){
      /* Put the control back where it was - the select has already moved. */
      syncModeControls();
      return;
    }
  }
  ...
  /* Carry the clock over ONLY if it is still the outgoing mode's default. A club
     ...
     and the confirm never mentioned. When they are left alone the template no
     longer lands on the target end time, which is exactly what the end-check in
     the toolbar and Balance Segments are for. */
  const from = MODE_TIMES[cur], to = MODE_TIMES[next];
  if(from && to
     && state.meeting.startTime === from.startTime
     && state.meeting.endTime === from.endTime){
    state.meeting.startTime = to.startTime;
    state.meeting.endTime = to.endTime;
  }
  editingSegId = null;
  expandedSegs.clear();
  syncLanguageEvaluatorSegment();
  ...
  renderPreviewNow();
  saveState();
  showBanner(next === 'contest'
    ? 'Contest running order loaded. Fill the Contest Committee under Contest Appointments, then add contestants to each contest block.'
    : 'Chapter-meeting running order loaded.', false);
}

/* True when the running order is still exactly what the mode's template lays
   down, so switching away from it destroys nothing and needs no confirm.

   This compares the WHOLE segment rather than a list of fields, and that is the
   entire point. The first version enumerated eight fields it thought mattered
   and missed five that a user can edit - pathway, level, the three timing
   lights, signalsManual, the per-segment sub-note and the flex range. Editing
   any of those and switching mode wiped the running order with no prompt, and
   setMeetingMode() then autosaved the wiped order over the file on disk. A
   whitelist of fields is a promise to remember every field anyone ever adds;
   this cannot go stale.

   The comparison is deliberately BIASED TOWARD "not pristine": a false negative
   costs one confirm dialog the user clicks through, a false positive costs them
   their evening's work. So anything that differs at all - including a key the
   file carried and the template does not - counts as changed.
   `id` is excluded because it is allocated from a counter and never matches;
   `disabled` is normalised because syncRoleSegments() sets it to false on rows
   a freshly built template leaves absent. */
function segFingerprint(sg){
  const o = {};
  Object.keys(sg).sort().forEach(k=>{
    if(k === 'id') return;
    if(k === 'disabled'){ o.disabled = !!sg.disabled; return; }
    if(sg[k] === undefined) return;
    o[k] = sg[k];
  });
  if(!('disabled' in o)) o.disabled = false;
  return JSON.stringify(o);
}
function runningOrderIsPristine(mode){
  const tpl = buildModeSegments(mode);
  const cur = state.segments || [];
  if(cur.length !== tpl.length) return false;
  for(let i = 0; i < cur.length; i++){
    if(segFingerprint(cur[i]) !== segFingerprint(tpl[i])) return false;
  }
  return true;
}

/* The mode lives in two controls that must agree: the selector in Meeting Info
   and the body class the form pane uses to show one mode's sections and hide the
   other's. */
function syncModeControls(){
  const sel = document.getElementById('f-mode');
  if(sel) sel.value = isContest() ? 'contest' : 'chapter';
  document.body.classList.toggle('mode-contest', isContest());
  document.body.classList.toggle('mode-chapter', !isContest());
}

/* ================= The roles grid, rendered (V43) =================
   It was static markup for nine built-in roles until the contest mode needed a
   different set in the same place. Rendering it from modeRoleLabels() keeps ONE
   grid rather than two that drift, and every id it emits - r-KEY, rc-KEY, rw-KEY
   - is the convention syncFormInputs() already fills, so nothing downstream
   changed. */
...
/* ================= Contestants (V43) =================
   One editor per contest block in the running order, found by walking the
   segments rather than by a fixed pair of lists: a club running three contests
   in one sitting adds a third block from the Add bar and gets a third editor,
   and one that runs a single contest deletes a block and the editor goes with
   it. The block's title is edited here too, which is what retitles the shipped
   "Table Topics Contest" into "Humorous Speech Contest" for the other half of
   the year. */
function contestBlocks(){ return (state.segments || []).filter(s => s.isContestants && !s.disabled); }

function renderContestants(){
  const box = document.getElementById('contestList');
  if(!box) return;
  const blocks = contestBlocks();
  if(!blocks.length){
    box.innerHTML = `<div class="hint">No contest blocks in the running order. `
      + `Add one from <b>Programme Segments</b> (&ldquo;Contest (contestant list)&rdquo;).</div>`;
    return;
  }
  box.innerHTML = blocks.map(seg=>{
    const rows = seg.contestants.map((name, i)=>`
      <div class="ct-row">
        <span class="ct-num">${i+1}</span>
        <input type="text" placeholder="Contestant name — leave blank for TBD"
          value="${esc(name)}" oninput="updContestant('${seg.id}',${i},this.value)">
        <button class="ct-del" title="Remove this contestant"
          aria-label="Remove contestant ${i+1}" onclick="removeContestant('${seg.id}',${i})">✕</button>
      </div>`).join('');
    return `<div class="ct-block" data-ct-id="${seg.id}">
      <label>Contest name</label>
      <input type="text" class="ct-title" value="${esc(seg.title)}"
...
/* Typing does NOT re-render the editor. The input already holds the character
   the user just typed, so redrawing it would move the caret to the end of the
   field on every keystroke - the same reason liveEdit() syncs the sheet rather
   than rebuilding the form. Only add and remove change the row COUNT, and only
   those redraw. */
function updContestant(segId, idx, val){
  const seg = state.segments.find(s=>s.id===segId);
  if(!seg || !Array.isArray(seg.contestants) || idx < 0 || idx >= seg.contestants.length) return;
  seg.contestants[idx] = val;
  updatePreview();
}
function bindContestTitle(segId, val){
  const seg = state.segments.find(s=>s.id===segId);
  if(!seg) return;
  seg.title = val;
  renderSegmentsList();
...
function addContestant(segId){
  ...
  /* Put the caret in the row that was just created - otherwise adding five
     contestants is five clicks and five hunts for the new field. */
  const rows = document.querySelectorAll(`.ct-block[data-ct-id="${segId}"] .ct-row input`);
  const last = rows[rows.length - 1];
  if(last) last.focus();
}
function removeContestant(segId, idx){
  const seg = state.segments.find(s=>s.id===segId);
...
function updateSeg(id, key, value){
  ...
  } else if(seg.isContestants && key === 'title'){
    /* Same mirroring a speech card gets: the Contestants section carries its own
       copy of this block's name, and without this it kept showing the old one -
       so the next keystroke in that field wrote the stale title back over the
       rename. Guarded on the title because nothing else here is duplicated. */
    renderContestants();
  }
  updatePreview();
  const {clockById} = computeSchedule();
...
/* ================= The Add lists (V43) =================
   Both add bars were the same 21 hardcoded <option>s, written out twice. They
   are rendered now because the contest running order needs a different list in
   the same control, and one list is one place to add the next item rather than
   two that have to be kept in step by hand. (V42's two copies were in fact
   identical - checked entry for entry before this was written.)
   Labels here are the ADD-MENU wording, which is not always the preset's label:
   the menu is read as a list of things you might insert, the sheet as a sentence
   about what happens next. */
const ADD_OPTIONS = {
  chapter: [
    ['custom','Custom item'],
    ['registration','Registration & Fellowship'],
    ...
    ['breaktime','Break Time (flexible)'],
    ['ttmasterintro','TME Introduces Table Topics Master'],
    ['tabletopics','Conduct Table Topics Session (flexible)'],
    ['ttreturn','TT Master Returns Control to TME'],
    ['ttvote',"Timer's Report | Voting for Best Table Topics Speaker"],
    ['peintro','TME Introduces Project Evaluation Segment'],
    ['evalvote',"Timer's Report | Voting for Best Speech Evaluation"],
    ['timerreport',"Call for Timer's Report (on its own)"],
    ['reports','TME Calls for Reports'],
    ['awards','Award Presentation'],
    ['closing',"President's Closing Address"],
    ['photo','Photo Taking Session'],
    ['adjourn','President Adjourns Meeting'],
  ],
  contest: [
    ['custom','Custom item'],
    ['contest','Contest (contestant list)'],
    ['csetup','Set up of Room by SAA'],
    ['cbriefchair','Briefing for Contest Chair & Photographer'],
    ['cbriefjudges','Briefing for Judges, Tally Counters & Timers'],
    ['cdraw','Contestants to Draw Ballots'],
    ['ccalltoorder','SAA Calls Contest to Order'],
    ['copening','Opening Address'],
    ['crules','Briefing of Contest Rules'],
    ['ctestspeech','Test Speaker Speech'],
    ['cholding','Contestants Proceed to Holding Room'],
    ['ccollect',"Collection of Judges' Ballots & Timers' Record"],
    ['cbreak','Refreshment Break (flexible)'],
    ['ccerts','Presentation of Certificates of Participation'],
    ['cappreciation','Certificates of Appreciation'],
    ['cresults','Announcement of Results'],
    ['cclosing','Closing Address'],
    ['cphoto','Photography Session'],
  ],
};
function renderAddOptions(){
  const list = ADD_OPTIONS[isContest() ? 'contest' : 'chapter'];
  const html = list.map(([k,label])=>`<option value="${k}">${esc(label)}</option>`).join('');
  ['addSegTypeTop','addSegType'].forEach(id=>{
    const sel = document.getElementById(id);
    if(!sel) return;
    /* Keep the user's choice across a re-render when the new list still has it,
       so switching mode and back does not silently re-point a half-made click. */
    const want = sel.value;
    sel.innerHTML = html;
    if(want && list.some(([k])=>k===want)) sel.value = want;
  });
}
...
function addSeg(atTop){
  ...
  renderSegmentsList();
  /* A contest block added here needs its editor to appear in the Contestants
     section, which renderSegmentsList() alone does not draw. */
  if(seg.isContestants) renderContestants();
  updatePreview();
  const card = document.querySelector(`.seg-card[data-seg-id="${seg.id}"]`);
...
function syncFormInputs(){
  /* The mode decides which grid is drawn, so it has to be settled first; the
     built-in and custom role rows must both exist before the loop below tries
     to fill them. */
  syncModeControls();
  renderAddOptions();
  renderRolesGrid();
