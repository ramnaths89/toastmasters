...
const SHEET_CSS = `
  ...
  :root{
    ...
    --brand-bleed: 1.2mm;   /* see .pane-brand in the print block */
    --print-foot: 6.5mm;
    --print-head: 33mm;
  }
  *{ box-sizing: border-box; }
  ...
  /* Contestants on a contest row (V43), numbered in draw order. Same two-column
     grid as the roster above it so the two blocks sit on the same rhythm, with
     the number in the label gutter instead of an appointment.
     Deliberately ONE column and no column-count: html2canvas fragments a
     multi-column box and the export is the deliverable, which is exactly the
     seam that cost V37-V39 - see the export-seam note. If a contest list ever
     needs to be shorter on paper, tighten the print block below, do not flow it.
     A number is not followed by a colon: "1: Sam Ong" reads as a score. */
  .contestant-list{
    display:grid;
    grid-template-columns: max-content 1fr;
    column-gap: 8px;
  ...
  /* ---- Neo Memphis Pop: Sottsass, the Memphis Group, an MTV bumper ----
     Flat colour, hard edges, offset blocks, nothing shaded. Happy Yellow is the
     brand anchor; coral and turquoise are the Memphis half. Deliberately built
     WITHOUT the things that have bitten this sheet before: no transform, no
     clip-path, no conic-gradient, no filter, no column-count - html2canvas
     supports none of them reliably and the export IS the deliverable here.
     The banner's pink band is a colour stop inside the same linear-gradient
     rather than an inset shadow, so header and .pane-brand paint it at exactly
     the same y and the top band stays one piece. */
  body.th-neomemphis{ --row-alt:#fdf2f6; --line:#d9cfd3; --cream:#fffcf3; }
  body.th-neomemphis .page{ background:#fffcf3; }
  body.th-neomemphis header, body.th-neomemphis .pane-brand{
    background: linear-gradient(to bottom, #17130f 0%, #17130f 94%, #ea5a8b 94%, #ea5a8b 100%);
    border-bottom: 5px solid var(--yellow);
  }
  body.th-neomemphis .head-text .club{
    color:#fff; font-weight:800; text-transform:uppercase;
    text-shadow: 2px 2px 0 #ea5a8b;
  }
  body.th-neomemphis .head-text .clubnum{ color:#5ecdc4; letter-spacing:1.4px; }
  body.th-neomemphis .head-text .meeting-title{
    color:#17130f; background:var(--yellow); display:inline-block;
    padding:1px 8px; font-style:normal; font-weight:700;
    box-shadow: 3px 3px 0 #1fa8a0;
  }
  body.th-neomemphis .meta-row b{ color:var(--yellow); }
  body.th-neomemphis thead th{ background:#1fa8a0; color:#17130f; font-weight:800; }
  body.th-neomemphis td.time{ color:#ea5a8b; font-weight:700; }
  body.th-neomemphis .item-title{ color:#17130f; }
  body.th-neomemphis tbody tr{ border-bottom-color:#e4dbdf; }
  body.th-neomemphis aside{ background:#fdf2f6; }
  body.th-neomemphis aside h3{
    color:#1fa8a0; border-bottom:3px solid #ea5a8b;
  }
  body.th-neomemphis .exco-role{ color:#17130f; font-weight:700; }
  body.th-neomemphis .theme-strip{
    background:#fff8dc; border-left:6px solid #1fa8a0;
  }
  body.th-neomemphis .poetts{ border-left-color:#ea5a8b; }
  body.th-neomemphis .rr-label{ color:#ea5a8b; }
  body.th-neomemphis .tbd{ border-radius:0; border-style:dashed; }
  body.th-neomemphis .flex-badge{
    background:var(--yellow); color:#17130f; border:2px solid #17130f; border-radius:0;
  }
  body.th-neomemphis .sig-boxes{ border:2px solid #17130f; border-radius:0; }
  body.th-neomemphis footer{ border-top:3px solid #17130f; }
  ...
  @media print{
    ...
    * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    aside.ref-pane{
      ...
      border-top: none;
      /* column-count: 1 REMOVED (V37). It was here as belt and braces against a
         stray multi-column rule that does not exist - nothing else in this file
         sets column-count, columns, column-gap (outside grid) or column-rule - and
         it was the cause of the vertical line down every exported PDF and JPG.
         Setting column-count makes the aside a MULTI-COLUMN formatting context. As
         soon as .pane-body is taller than the aside, Chrome fragments it into an
         OVERFLOW COLUMN laid out to the RIGHT, one column-width across plus a
         column-gap of 1em (the aside is 10.5px, so 10.5px). getBoundingClientRect()
         returns the UNION of an element's fragments, and html2canvas positions
         element boxes from getBoundingClientRect while positioning TEXT from
         per-text-node rects. So the pane's copy stayed where it belonged and its
         border-right was painted at the union's right edge:
             172.34 + 10.5 + 172.34 = 355.1 CSS px = 49.3% of the sheet,
         top 0 to the aside's bottom - a 1px line of var(--line) down the whole
         page, over the banner and the table head alike. The real pane rule at
         171.8 vanished at the same time, because it is the same border, displaced.
         Reproduced end to end and confirmed by hooking ctx.fill(): a clean export
         logs exactly one tall-thin fill, at x 171.34. */
      font-size: 10.5px;
      background: #fff;
    }
    ...
    .pane-brand{
      ...
      /* Bleeds --brand-bleed past the pane's right edge, and takes that back as
         padding so the logo stays centred on the pane, not on the bleed (V41).
         The two halves of the top band overlap by 0.4mm on the header's side and
         that is enough for the browser - but html2canvas does not honour it. It
         lays the banner out at the pane boundary regardless of the negative
         margin, so brand and banner ABUT there, and a sub-pixel gap let the white
         page through as a pale 1px line between the logo and the club name in
         every PDF and JPG. Measured with each half force-coloured: red stopped at
         device x=358, green began at 360, and 359 was neither. Widening the
         overlap does nothing - the library ignores it - so the brand carries its
         own gradient across the join instead. It only ever paints there in the
         export: on screen and on paper aside.ref-pane is overflow:hidden and
         clips the bleed away, so this is invisible everywhere else. */
      width: calc(var(--print-pane) + var(--brand-bleed));
      padding: 2mm calc(2mm + var(--brand-bleed)) 2mm 2mm;
      box-sizing: border-box;
    }
    ...
    /* Left padding was ZERO: the pane's text sat hard against the paper's own
       margin with nothing between it and the edge, while the agenda beside it had
       6mm of air. Rama on the JPG: "the components on the left are too far to the
       left". 3.5mm each side gives the column an inset on both flanks and lines
       its left edge up with nothing in particular, which is the point - it should
       read as a column of type, not as text that fell off the page. It costs
       3.5mm of measure, and the V31 compaction left ~52mm of headroom to spend. */
    .pane-body{
      padding: 4mm 3.5mm 0 3.5mm;
      border-right: 1px solid var(--line);
    ...
    /* Turned again (V30). The Language Evaluator role adds a 26th row and the
       filled sheet was already reaching a third page in classic, zine and swiss,
       so there was nothing left to absorb it. A pixel off each side of 26 rows is
       still the cheapest space on the sheet, and the row rules do the separating,
       so nothing reads as crowded. The leading comes down with it: 1.42 was
       chosen for a screen read, and a table of one- and two-line entries does not
       need that much air between lines that rarely wrap. */
    ...
    /* The POETTS block under a prepared speech and the role roster under a
       ...
       five or six short label/value pairs, each on its own grid row, each with a
       1px gutter and 1.45 leading meant for the screen. They are read one line at
    ...
    /* The theme strip is one line of text in a tinted box. On screen it wants air
       around it; on paper it is a caption above the agenda and the 14px margin
       below it was reading as a gap, not as separation. */
    .theme-strip{ padding: 4px 12px; margin-bottom: 8px; font-size: 11.5px; }

    /* The pane does not behave like the agenda when it overruns. It is fixed, one
       page tall and overflow:hidden, so there is no page 3 to spill onto and no
       warning: the bottom is simply cut off, and what sits at the bottom is
       Announcements - the only block on the sheet written the same week it is
       read. Rama has lost it in the post more than once.
       Every number below was sized for a 200px screen sidebar and then inherited
       into a 45.6mm printed column where the same sentence wraps three times, so
       the leading and the gutters were paying for air the paper does not need.
       Leading is cut first, because it costs nothing to read back. Type size is
       cut only where the words are reference matter - the Pathways glossary, the
       URLs, the parenthetical home club after an officer's name - and never on
       the officers' own names, which are the reason the pane exists.
       Measured against the page BOX, not the viewport: a full night with four
       announcements went from 23mm of headroom to 52mm, and a stress sheet of
       five speeches and six announcements from 6mm to 38mm. Six is one wrapped
       line away from losing text; thirty-eight is a real margin.
       It is not unlimited. A pane carrying the full District officer set, four
       links AND eight announcements still overruns by about 9mm, and no amount
       of leading fixes that - 281mm of material does not go into 234mm. That one
       wants a warning in the editor or a second column, not a smaller typeface. */
    aside h3{ font-size: 9.5px; line-height: 1.25; margin-bottom: 6px; padding-bottom: 3px; }
    .exco-block{ margin-bottom: 9px; break-inside: avoid; }
    .exco-item{ margin-bottom: 3.5px; }
    /* Role over name is one unit, not two lines of a list: closing the leading
       between them groups the pair and buys back 8 lines across the two blocks. */
    .exco-role{ font-size: 9.5px; line-height: 1.22; }
    .exco-name{ font-size: 10px; line-height: 1.28; }
    .exco-sub{ font-size: 8.5px; line-height: 1.22; }
    /* The only thing left in the pane still set at the aside's inherited 10.5px,
       and the one thing nobody reads a character at a time: a URL that already
       breaks mid-word in a 41mm column. */
    .links a{ font-size: 9px; line-height: 1.3; }
    .path-legend div{ font-size: 9px; line-height: 1.24; margin-bottom: 1.5px; }
    .announcements .announce-line{ font-size: 9px; line-height: 1.3; margin-bottom: 2.5px; }
    .holder-pair .hp-line{ line-height: 1.3; font-size: 10px; }
    .announcements .announce-gap{ height: 2.5px; }



    /* ---- One density for every theme (V37) ----
       This used to be five separate patches, one per theme, each measured against
       the two-page boundary on its own and each leaving the others alone. Two
       things were wrong with that. The obvious one: the printed sheet came out a
       different size in different themes, which is what Rama saw and asked to be
       made uniform. The buried one: three of those patches sat immediately after
       an ORPHANED declaration left behind when a retired theme's selector was
       deleted and its body was not -

           /@ Confessional sets a monospace body face ... @/
             font-family: "Source Sans 3", ...;
           }

       A stray declaration at rule level makes the CSS parser discard tokens until
       it can resynchronise, and it took the following rules with it. Measured in
       the built file: 52 selectors written in the print block, 46 actually built,
       with one theme's item-title compaction among the casualties - so a theme
       whose own comment read "measured: tracking alone left it on three pages"
       was, in the shipping build, not being compacted at all. (That theme was
       Retro-Futurism, retired in V42; named here in prose rather than as a live
       selector so a grep for a dead theme stays clean.)
       Everything below applies to every theme. A theme may still differ in COLOUR
       and RULE WEIGHT on paper; it may not differ in how much paper it uses.
       Written with a body[class] prefix, which is the trick already used on .page
       further up this block: a bare 'tbody tr' loses to 'body.th-zine tbody tr'
       on specificity, so the first version of these rules left Zine and Brutalist
