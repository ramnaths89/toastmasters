...
/* PNG has no quality dial, so the only lever is resolution: encode, and if it is
   over budget step the width down and try again. Lossless keeps the 9.5px pane
   type crisper than JPG at the same byte size, which is why it is worth offering -
   but a full sheet often cannot reach 500 KB as PNG, so this reports honestly
   rather than pretending, and points at the JPG instead. */
async function encodePngUnder(canvas, target){
  const widths = [MAX_EXPORT_WIDTH, 1300, 1150, 1000, 900, 800].filter(w => w <= canvas.width);
  if(!widths.length) widths.push(canvas.width);
  let last = null;
  for(const w of widths){
    const c = (w === canvas.width) ? canvas : downscaleCanvas(canvas, w);
    const blob = await new Promise(r => c.toBlob(r, 'image/png'));
    if(!blob) continue;
    last = {blob, w: c.width, h: c.height};
    if(blob.size <= target) return last;
  }
  return last;
}
...
async function downloadImage(kind){
  ...
  try{
    ...
    const out = png ? await encodePngUnder(canvas, IMAGE_TARGET_BYTES)
                    : await encodeJpegUnder(canvas, IMAGE_TARGET_BYTES);
    if(!out) throw new Error('encode');
    saveBlob(out.blob, sheetFileStem() + (png ? '.png' : '.jpg'));
    const kb = Math.round(out.blob.size / 1024);
    showBanner('Saved a ' + out.w + '\u00d7' + out.h + (png ? ' PNG (' : ' JPG (') + kb + ' KB).'
      + (out.blob.size > IMAGE_TARGET_BYTES
          ? ' That is over the 500 KB cap — the sheet is unusually long this week'
            + (png ? '; the JPG option will come in smaller.' : '.')
          : ''), out.blob.size > IMAGE_TARGET_BYTES);
  }catch(err){
